...
class RefreshToken(Base):
    ...
    __table_args__ = (
        Index("ix_refresh_tokens_family_revoked", "family_id", "revoked"),
    ...
    family_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True, nullable=True, default=uuid.uuid4)
...
class Booking(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("profiles.id", ondelete="SET NULL"), index=True, nullable=True)
    ...
    deleted_by_user_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("user_auth.id", ondelete="SET NULL"), nullable=True
    )
    deleted_by_email: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True)
    deleted_by_role: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
...
class StaffAssignment(Base):
    ...
    staff_user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("user_auth.id", ondelete="CASCADE"), index=True, nullable=False)
...
class NotificationRecord(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    recipient_email: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True)
    recipient_phone: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True)
